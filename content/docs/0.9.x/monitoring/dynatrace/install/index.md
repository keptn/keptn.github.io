...
## Deploy Dynatrace OneAgent Operator on Kubernetes
...
- To verify that the OneAgent is working properly, check the status of the pods, e.g. using kubectl:

  ```console
  kubectl get pods -n dynatrace
  ```
  A status of `Running` indicates that the Dynatrace OneAgent Operator is functioning normally, while `Error` or `CrashLoopBackOff` are signs of a problem.
