...
```
helm install keptn https://github.com/keptn/keptn/releases/download/0.11.0/keptn-0.11.0.tgz -n keptn --create-namespace --wait --set=continuous-delivery.enabled=true,control-plane.apiGatewayNginx.type=LoadBalancer
```

* *Install Keptn execution-plane:*

```
helm install jmeter-service https://github.com/keptn/keptn/releases/download/0.11.0/jmeter-service-0.11.0.tgz -n keptn --create-namespace --wait

helm install helm-service https://github.com/keptn/keptn/releases/download/0.11.0/helm-service-0.11.0.tgz -n keptn --create-namespace --wait
```
...
| `control-plane.bridge.lookAndFeelUrl`                 | Specifies a [custom look and feel](https://github.com/keptn/keptn/tree/release-0.11.0/bridge#custom-look-and-feel) for Keptn Bridge                                             | any URL containing a zip archive                                                                 | null                                  |
...
| `control-plane.mongo.enabled`                         | Allows to not install the default MongoDB instance in Keptn.                                                                                                                    | true,false                                                                                       | true                                  |
| `control-plane.mongo.architecture`                    | MongoDB Architecture                                                                                                                                                            | `standalone` or `replicaset`                                                                     | `standalone`                          |
| `control-plane.mongo.service.nameOverride`            | Override for the kubernetes service name for MongoDB                                                                                                                            | any string                                                                                       | mongo                                 |
| `control-plane.mongo.service.port`                    | Port on which the the MongoDB kubernetes service is reachable                                                                                                                   | Port number                                                                                      | 27017                                 |
| `control-plane.mongo.auth.database`                   | Database that should be created during initialization                                                                                                                           | any string                                                                                       | keptn                                 |
| `control-plane.mongo.auth.existingSecret`             | Secret that is used for credentials of MongoDB (If the password and rootPassword values below are used, remove this field)                                                      | any string                                                                                       | `mongodb-credentials`                 |
| `control-plane.mongo.auth.username`                   | Custom user that should be created during initialization                                                                                                                        | any string                                                                                       | keptn                                 |
| `control-plane.mongo.auth.password`                   | Password for the custom user that is created during initialization                                                                                                              | any string                                                                                       | empty                                 |
| `control-plane.mongo.auth.rootUser`                   | MongoDB root user                                                                                                                                                               | any string                                                                                       | admin                                 |
| `control-plane.mongo.auth.rootPassword`               | MongoDB root password                                                                                                                                                           | any string                                                                                       | empty                                 |
| `control-plane.mongo.external.connectionString`       | If an external MongoDB instance is used, specify the full connection string for that instance here.                                                                             | any string                                                                                       | empty                                 |
...
```console
helm upgrade keptn keptn --install -n keptn --create-namespace --wait --version=0.11.0 --repo=https://storage.googleapis.com/keptn-installer --set=control-plane.apiGatewayNginx.type=LoadBalancer
```
...
```console
helm upgrade keptn keptn --install -n keptn --create-namespace --wait --version=0.11.0 --repo=https://storage.googleapis.com/keptn-installer --set=continuous-delivery.enabled=true
```
...
```console
helm upgrade keptn keptn --install -n keptn --create-namespace --wait --version=0.11.0 --repo=https://storage.googleapis.com/keptn-installer --set=continuous-delivery.enabled=true,control-plane.apiGatewayNginx.type=LoadBalancer
```
...
Download the Helm charts from the [Keptn 0.11.0 release](https://github.com/keptn/keptn/releases/tag/0.11.0):

* Keptn Control Plane: https://github.com/keptn/keptn/releases/download/0.11.0/keptn-0.11.0.tgz
* helm-service (if needed): https://github.com/keptn/keptn/releases/download/0.11.0/helm-service-0.11.0.tgz
* jmeter-service (if needed): https://github.com/keptn/keptn/releases/download/0.11.0/jmeter-service-0.11.0.tgz
...
mkdir offline-keptn
cd offline-keptn
curl -L https://github.com/keptn/keptn/releases/download/0.11.0/keptn-0.11.0.tgz -o keptn-0.11.0.tgz
curl -L https://github.com/keptn/keptn/releases/download/0.11.0/helm-service-0.11.0.tgz -o helm-service-0.11.0.tgz
curl -L https://github.com/keptn/keptn/releases/download/0.11.0/jmeter-service-0.11.0.tgz -o jmeter-service-0.11.0.tgz
cd ..
```
...
```console
cd offline-keptn
curl -L https://raw.githubusercontent.com/keptn/keptn/0.11.0/installer/airgapped/pull_and_retag_images.sh -o pull_and_retag_images.sh
chmod +x pull_and_retag_images.sh
KEPTN_TAG=0.11.0 ./pull_and_retag_images.sh "your-registry.localhost:5000/"
cd ..
```
...
```console
cd offline-keptn
curl -L https://raw.githubusercontent.com/keptn/keptn/0.11.0/installer/airgapped/install_keptn.sh -o install_keptn.sh
chmod +x install_keptn.sh
./install_keptn.sh "your-registry.localhost:5000/" keptn-0.11.0.tgz helm-service-0.11.0.tgz jmeter-service-0.11.0.tgz
cd ..
```
...
```console
helm upgrade keptn keptn --install -n keptn --create-namespace --wait --version=0.11.0 --repo=https://storage.googleapis.com/keptn-installer --set=control-plane.apiGatewayNginx.type=LoadBalancer,continuous-delivery.enabled=true,control-plane.prefixPath=/mykeptn
```
