...
If you want to install Keptn just to explore the capabilities of quality gates, you have the option to roll-out Keptn **without** components for automated delivery and operations. Therefore, set the `use-case` flag to `quality-gates` when executing the [install](../../cli/commands/keptn_install) command:
...
```
  ...
  **Note:** Since you are not actively deploying a service in this tutorial, [keptn create service](../../cli/commands/keptn_create_service) does not require you to provide a Helm chart compared to the [keptn onboard service](../../cli/commands/keptn_onboard_service) command. 
...
* Execute a quality gate evaluation by using the Keptn CLI to [send event start-evaluation](../../cli/commands/keptn_send_event_start-evaluation): 
...
* Retrieve the evaluation results by using the Keptn CLI to [get event evaluation-done](../../cli/commands/keptn_get_event_evaluation-done): 
