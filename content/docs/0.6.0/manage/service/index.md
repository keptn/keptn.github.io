...
**Note:** If you need to store resources (e.g., test files, configuration files, etc.) that are required by a service, use the [add-resource](../../reference/cli/commands/keptn_add-resource) command and specify the `--project`, `--stage`, and `--service` as shown below:
