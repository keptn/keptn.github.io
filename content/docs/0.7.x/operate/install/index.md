...
1. Download, install [K3s](https://k3s.io/) (tested with [versions 1.16 to 1.19](../k8s_support)) and run K3s using the following command:
   ```console
...
* The **Control Plane with the Execution Plane (for Continuous Delivery)** allows to implement [Continuous Delivery](../../../previous-releases/concepts/delivery/) on top of *quality gates* and *automated operations* use cases. To install the control plane with the execution plane for continuous delivery, execute:
